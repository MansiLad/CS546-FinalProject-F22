...
const validatePass = (password) => {
  ...
  if(!/(?=.*\d)(?=.*[a-z])(?=.*[A-Z])((?=.*\W)|(?=.*_))^[^ ]+$/.test(password)){
...
router
  ...
  .post(async (req, res) => {
    ...
    try{
      ...
      return res.status(500).render('userRegister', {title: 'Register', error:e });
    }
...
router
  ...
  .get(async (req, res) => {
    //code here for GET
    if (!req.session.user) {
      res.status(403).render('forbiddenAccess', {title: 'Access Denied', error:'Access Denied'});
    }else{
  ...
  .get(async (req, res) => {
    ...
    req.session.destroy();
  //res.send('Logged out');
   return res.render("logout", { title: "Logout", message: "User has been successfully logged out"});
